...
def buscar_en_pdfs(consulta):
    ...
    for idx, score in enumerate(cosine_scores):
        if score.item() > 0.25:  # Filtro mínimo de similitud
...
def procesar_consulta(consulta, seleccion_previa=None):
    ...
    if inventario_habilitado:
        ...
        coincidencias = [p for p in productos_disponibles if consulta_limpia in p.lower()]
    
    # Menú dinámico de aproximación lógica (Ej: escribe 'Arroz' -> devuelve listado múltiple)
    ...
    # Obtener el registro único seleccionado o identificado del inventario
    ...
    # Armar planilla HTML de Inventario de forma dinámica (soporta cambios en los nombres de columnas)
    html_inventario = ""
    ...
    resultados_pdf = buscar_en_pdfs(consulta)
    html_pdfs = ""
    if resultados_pdf:
        html_pdfs = f"""
        <div>
